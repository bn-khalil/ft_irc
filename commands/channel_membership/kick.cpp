...
/*   Updated: 2025/11/28 11:33:07 by akella           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::kick(Client &c)
{
    ...
    std::map<std::string, Channel>::iterator it = channel.find(cmds[1]);
    
    if (name_perfect(cmds, it))
    {
    ...
    {
        kick_with_brodcast(it, cmds, reason);
    }
}